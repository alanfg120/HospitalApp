...
class _SolicitarCitaPageState extends State<SolicitarCitaPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (stateUsuario is AutenticadoState) usuario = stateUsuario.usuario;

    return Scaffold(
           key    : _scaffoldKey,
           appBar : AppBar(
                    title           : Text("Solicite tu Cita", style: TextStyle(color: Colors.white,fontSize: 20)),
                           ...
                           => GestureDetector(
                              ...
                              child: SingleChildScrollView(
                                     child: Container(
                                            ...
                                            child   : Column(
                                                      children: <Widget>[
                                                                  Text("Selecione Tipo de Cita",style: TextStyle(fontSize: 18)),
                                                                  SizedBox(height: 20),
                                                                  ..._opcionesCitas(citas),
                                                                  Text("Asignacion Automatica",style: TextStyle(fontSize: 18)),
                                                                  SizedBox(height: 20),
                                                                  _asigtauto(),
                                                                  SizedBox(height: 20),
                                                                  _observaciones(),
  ...
  List<Widget> _opcionesCitas(List citas) {
    return citas.asMap().entries.map((item) 
            => Row(
               children: <Widget>[
                            Radio(
                            value       : item.key,
    ...
    return Row(
           children        : <Widget>[
                               ...
                               child:  FractionallySizedBox(
                                       widthFactor : 0.6,
                                       child       : Text("Si esta Opcion esta Activa la cita sera Asignada Atomaticamente")
                                       )
                               ),
                               Switch(
                               value       : swicht,
                               onChanged   : (value)=>setState(()=>swicht = value),
                               activeColor : Color(0xFF01C6BD),
                               )
           ],
  ...
  Widget _observaciones() {
    return TextField(
           ...
           decoration   : InputDecoration(
                          ...
                          enabledBorder : OutlineInputBorder(borderSide: BorderSide(color: Color(0xFF01C6BD)))
                          ),
   );
  }
  ...
  Widget _solicitarBottom(BuildContext context, Usuario usuario) {
    return MaterialButton(
           ...
           child    : Text("Solicitar",style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  dialogoNewCita(BuildContext context) {
    ...
    builder: (context) 
              => AlertDialog(
                 ...
                 elevation       : 0,
                 title           : Text("Solicitando Turno Espere", style: TextStyle(color: Colors.white)),
                 content         : Container(
                                   height    : 35,
  ...
  _snakbar(String mensaje) {
    ...
    ..closed.then((data) {
        if (mensaje != errormessaje) Navigator.pop(context);
      });
  }
}
